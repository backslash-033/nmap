...
static bool opt_speed(options *opts, str arg) {
	...
	if (thread_nb > 250 || thread_nb < 1) {
		fprintf(stderr, WARNING "--speedup can only be between 1 and 250 included. "
						"Argument given: %d. "
...
static bool opt_ports(options *opts, str arg) {
	...
	for (int i = 0; splitted[i]; i++) {
		...
		if (!range_size_value && (unsigned int)errno == RANGE_ALLOCERR) {
			...
			fprintf(stderr, ERROR "Invalid port range: The port range has to be between 0 and 65535 included.\n");
			free_darray((void **)splitted);
			return true;
...
static void print_help_message() {
	...
	puts("--fast         Scans the 100 most scanned ports.");
	puts("--timeout      Timeout in seconds for each scans. Default: TCP: 2 UDP: 4");
